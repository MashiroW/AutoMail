"""Pipeline de traitement d'un PDF déposé dans l'inbox."""
...
def _unique(path: Path) -> Path:
    """Évite d'écraser un fichier existant en suffixant _2, _3…"""
    if not path.exists():
        return path
...
def process_one_file(conn: sqlite3.Connection, cfg: Config, src: Path) -> int | None:
    ...
    try:
        ...
        if existing is not None:
            dest = _unique(
                cfg.originals_dir / "duplicates" / f"{sha[:8]}_{safe_name(original_filename)}"
            )
...
def _handle_failure(conn, cfg: Config, src: Path, original_filename: str,
    ...
    try:
        ...
        # même sha déjà présent : on ne crée pas de doublon d'échec
        return None
